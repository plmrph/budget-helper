<script>
      ...
      if (budgetsResponse.success) {
        ...
        if (budgets.length > 0) {
          ...
          if (
            ...
            console.log("Using selected budget from config:", targetBudgetId);
          } else {
          ...
          if (targetBudget) {
            ...
            console.log("Selected budget:", targetBudget);
          } else {
            console.warn(
  ...
  async function prepareTrainingData() {
    ...
    try {
      ...
      console.log("Data preparation completed, clearing loading state");
...
async function pollTrainingStatus(modelNameToPoll) {
  ...
  const poll = async () => {
    try {
      ...
      if (statusResponse.success) {
        ...
        console.log(`Training status for ${modelNameToPoll}:`, status);
          ...
          if (consecutiveErrorCount >= maxConsecutiveErrors) {
            ...
            try {
              ...
              if (deleteResponse.success) {
                console.log(`Successfully deleted stuck model: ${modelNameToPoll}`);
              } else {
    ...
    try {
      ...
      if (
        ...
        const currentTrainingModel =
          currentTrainingResponse.data.current_training;
        console.log("Found ongoing training for model:", currentTrainingModel);
      ...
      for (const model of modelsList) {
        try {
          ...
          if (
            ...
            console.log("Found ongoing training for model:", model.name);
  ...
  async function deleteModel(model) {
    try {
      ...
      if (response.success) {
        ...
        await bulkMLPredictStore.checkDefaultModel();
      } else {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data?.settings?.['system.default_model_name']?.value?.stringValue) {
          ...
          console.log(`Current default model: ${defaultModelName}`);
        }
      }
    } catch (err) {
      console.log('Could not load default model name:', err);
    }
  }
  ...
  async function loadCategoryMapping(budgetId) {
    try {
      console.log("Loading category mapping for budget:", budgetId);
      ...
      if (
        ...
        console.log(`Loaded ${newCategoryMap.size} categories for mapping`);
      } else {
        console.warn(
  ...
  async function selectExistingDataset(dataset) {
    try {
      console.log("Selecting existing dataset:", dataset.id);
